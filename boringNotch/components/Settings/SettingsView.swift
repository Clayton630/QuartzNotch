...
struct SettingsView: View {
    private enum SettingsTab: String, CaseIterable, Identifiable {
        ...
        var label: (title: String, systemImage: String) {
            ...
            case .toolbar: return ("Toolbar", "rectangle.topthird.inset.filled")
            case .media: return ("Media", "play.laptopcomputer")
            case .calendar: return ("Calendar", "calendar")
            case .hud: return ("HUDs", "dial.medium.fill")
            case .liveActivities: return ("Live Activities", "sparkle")
            case .pages: return ("Pages", "square.on.square")
            case .shelf: return ("Shelf", "books.vertical")
            case .shortcuts: return ("Shortcuts", "keyboard")
            case .advanced: return ("Advanced", "gearshape.2")
        ...
        NavigationSplitView {
            List(SettingsTab.allCases, selection: $selectedTab) { tab in
                NavigationLink(value: tab) {
                    Label(tab.label.title, systemImage: tab.label.systemImage)
                }
            }
...
struct ToolbarSettings: View {
    ...
    var body: some View {
        Form {
            Section {
                Defaults.Toggle(key: .toolbarEnabled) {
                    Text("Enable toolbar")
                }
...
@Default(.sliderColor) var sliderColor
    ...
    var body: some View {
        Form {
            Section {
                Picker("Media mode", selection: $playbackScope) {
                    ForEach(PlaybackScope.allCases) { scope in
