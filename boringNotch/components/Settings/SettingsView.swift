...
struct SettingsView: View {
        ...
        var searchTerms: [String] {
            ...
            case .media:
                return [
                    ...
                    "hide for media app only", "never hide", "show lyrics below artist name",
            ...
            case .calendar:
                return [
                    ...
                    "hide all-day events", "auto-scroll to next event", "always show full event titles",
            ...
            case .advanced:
                return [
                    ...
                    "enable window shadow", "corner radius scaling", "force liquid glass compatibility fallback",
                    "window rendering", "hide on screen capture", "app icon"
                ]
            case .about:
        ...
        var searchItems: [String] {
            ...
            case .advanced:
                return [
                    ...
                    "Force liquid glass compatibility fallback",
    ...
    private var searchResultSections: [SettingsSearchSection] {
        ...
        return SettingsTab.allCases.compactMap { tab in
            let matchingItems = tab.searchItems.filter { $0.localizedLowercase.contains(normalizedSearchText) }
...
private struct SettingsSidebarList: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSTableViewDataSource, NSTableViewDelegate {
        ...
        private func previewSelection(row: Int) {
            guard let tableView, tabs.indices.contains(row), tableView.selectedRow != row else { return }
            suppressSelectionCallback = true
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            while let nextEvent = window?.nextEvent(
                ...
                case .leftMouseUp:
                    ...
                    guard releaseRow == initialRow else { return }
    ...
    final class SidebarCategoryCellView: NSTableCellView {
        ...
        init(identifier: NSUserInterfaceItemIdentifier) {
            self.hostingView = NSHostingView(rootView: SettingsCategoryRow(tab: .basics))
        ...
        func configure(with tab: SettingsView.SettingsTab) {
            hostingView.rootView = SettingsCategoryRow(tab: tab)
        }
...
private struct SettingsSearchField: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> SidebarSearchFieldContainer {
        ...
        container.searchField.placeholderString = "Search"
                ...
                Section {
                    VStack(spacing: 0) {
                        VStack(spacing: 6) {
                            ...
                            Text("You are currently running QuartzNotch Alpha 0.3.")
                                .font(.subheadline)
                                .foregroundStyle(.secondary)
                                .frame(maxWidth: .infinity, alignment: .center)
                                .multilineTextAlignment(.center)
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                .liveAccentToggleTint()
                Toggle("Force liquid glass compatibility fallback", isOn: $forceLiquidGlassCompatibilityFallback)
                    .liveAccentToggleTint()
            } header: {
