...
final class BluetoothActivityManager {
  ...
  // Anti-spam guard (avoids multiple popups for nearby events)
    ...
    private init() {
    // Boot behavior:
    // - Don't force a Bluetooth permission prompt at app launch.
    // - Start immediately only when authorization is already granted.
        ...
        bluetoothEnabledCancellable = Defaults.publisher(.bluetoothLiveActivityEnabled, options: [])
            .sink { [weak self] change in
                ...
                if change.newValue {
                // User explicitly enabled Bluetooth live activity: allow system prompt if needed.
                    self.start()
                } else {
                    self.stop()
                }
