                ...
                do {
                    ...
                    await MainActor.run { [weak self] in
                        ...
                        if updatedState.isPlaying {
                            let progressed = updatedState.currentTime + (now.timeIntervalSince(updatedState.lastUpdated) * updatedState.playbackRate)
                            updatedState.currentTime = min(max(0, progressed), updatedState.duration)
                        }
