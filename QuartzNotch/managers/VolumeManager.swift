...
final class VolumeManager: NSObject, ObservableObject {
  ...
  // MARK: - Public Control API
    @MainActor func increase(stepDivisor: Float = 1.0) {
        ...
        setAbsolute(target)
        QuartzViewCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: CGFloat(target))
    }
    ...
    @MainActor func toggleMuteAction() {
        ...
        toggleMuteInternal()
        QuartzViewCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: CGFloat(willBeMuted ? 0 : resultingVolume))
    }
