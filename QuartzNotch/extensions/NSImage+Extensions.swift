...
extension NSImage {
    ...
    func averageColor(completion: @escaping (NSColor?) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let sampleStep = max(1, min(width, height) / 160)
            ...
            let divisor = CGFloat(max(1, sampledCount))
            let averageRed = CGFloat(totalRed) / divisor / 255.0
            let averageGreen = CGFloat(totalGreen) / divisor / 255.0
            let averageBlue = CGFloat(totalBlue) / divisor / 255.0
    ...
    func artworkBarColors(count: Int, completion: @escaping (_ topColors: [NSColor], _ bottomColors: [NSColor]) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            let white = NSColor.white
            let fallback = Array(repeating: white, count: count)

            guard let cgImage = self.cgImage(forProposedRect: nil, context: nil, hints: nil) else {
                DispatchQueue.main.async { completion(fallback, fallback) }
                return
            }
            ...
            // top zone = top 45%, bottom zone = bottom 45%, slight gap in the middle for contrast
            let topZoneEnd = Int(CGFloat(height) * 0.45)
            let bottomZoneStart = Int(CGFloat(height) * 0.55)

            var topBins = Array(repeating: [Int: Int](), count: count)
            var bottomBins = Array(repeating: [Int: Int](), count: count)
            var globalSumR: UInt64 = 0
            var globalSumG: UInt64 = 0
            var globalSumB: UInt64 = 0
            var globalCount: UInt64 = 0

            var y = 0
            while y < height {
                let inTop = y <= topZoneEnd
                let inBottom = y >= bottomZoneStart
                let rowOffset = y * bytesPerRow
                var x = 0
                while x < width {
                    let col = min(count - 1, x * count / width)
                    let idx = rowOffset + (x * 4)
                    let r8 = bytes[idx + 0]
                    let g8 = bytes[idx + 1]
                    let b8 = bytes[idx + 2]
                    ...
                    if a8 > 12 {
                        ...
                        if !(bri < 0.08 && sat < 0.15) {
                            let key = Self.quantizedKey(r8: r8, g8: g8, b8: b8)
                            if inTop { topBins[col][key, default: 0] += 1 }
                            if inBottom { bottomBins[col][key, default: 0] += 1 }
                            globalSumR += UInt64(r8)
                            globalSumG += UInt64(g8)
                            globalSumB += UInt64(b8)
                            globalCount += 1
                        }
                    }
                    x += sampleStep
            ...
            guard globalCount > 0 else {
                DispatchQueue.main.async { completion(fallback, fallback) }
                return
            }

            let meanColor = NSColor(
                red: CGFloat(globalSumR) / CGFloat(globalCount) / 255.0,
                green: CGFloat(globalSumG) / CGFloat(globalCount) / 255.0,
                blue: CGFloat(globalSumB) / CGFloat(globalCount) / 255.0,
                alpha: 1
            )

            var globalBins: [Int: Int] = [:]
            for bins in topBins + bottomBins {
                for (k, v) in bins { globalBins[k, default: 0] += v }
            }

            var topColors: [NSColor] = []
            var bottomColors: [NSColor] = []

            for i in 0..<count {
                let topRaw = Self.bestDominantColor(in: topBins[i], avoiding: nil)
                    ?? Self.bestDominantColor(in: globalBins, avoiding: nil)
                    ?? meanColor
                let bottomRaw = Self.bestDominantColor(in: bottomBins[i], avoiding: nil)
                    ?? Self.bestDominantColor(in: globalBins, avoiding: nil)
                    ?? meanColor

                topColors.append(Self.makeReadableColor(from: Self.blend(topRaw, with: meanColor, amount: 0.20)))
                bottomColors.append(Self.makeReadableColor(from: Self.blend(bottomRaw, with: meanColor, amount: 0.20)))
            }

            DispatchQueue.main.async {
                completion(topColors, bottomColors)
            }
        }
    }
