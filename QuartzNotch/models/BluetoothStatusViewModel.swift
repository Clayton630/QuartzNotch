...
final class BluetoothStatusViewModel: ObservableObject {
    ...
    private let coordinator = QuartzViewCoordinator.shared
...
final class FocusModeLiveActivityManager: NSObject, ObservableObject {
    ...
    private func startAssertionsPolling() {
        ...
        timer.schedule(deadline: .now() + .seconds(1), repeating: .seconds(8), leeway: .seconds(1))
...
enum FullDiskAccessAuthorization {
    private static let knownAuthorizedKey = "QuartzNotchFullDiskAccessKnownAuthorized"
    private static let didProbeAuthorizationKey = "QuartzNotchFullDiskAccessDidProbe"
    ...
    static func hasPermission() -> Bool {
        if UserDefaults.standard.bool(forKey: knownAuthorizedKey) {
            return true
        }

        guard !UserDefaults.standard.bool(forKey: didProbeAuthorizationKey) else {
            return false
        }

        let status = refreshPermissionStatus(markAsProbed: true)
        return status.doNotDisturbDB || status.tccDB
    }

    @discardableResult
    static func refreshPermissionStatus(markAsProbed: Bool = false) -> (doNotDisturbDB: Bool, tccDB: Bool) {
        let status = readabilityStatus()
        UserDefaults.standard.set(status.doNotDisturbDB || status.tccDB, forKey: knownAuthorizedKey)
        if markAsProbed {
            UserDefaults.standard.set(true, forKey: didProbeAuthorizationKey)
        }
        return status
    }

    static func resetCachedPermissionStatus() {
        UserDefaults.standard.removeObject(forKey: knownAuthorizedKey)
        UserDefaults.standard.removeObject(forKey: didProbeAuthorizationKey)
    }

    static func readabilityStatus() -> (doNotDisturbDB: Bool, tccDB: Bool) {
