  ...
  // MARK: - Context Menu helpers (extracted from view)
    ...
    private final class MenuActionTarget: NSObject {
            ...
            if let appURL = sender.representedObject as? URL {
                ...
                Task {
                        ...
                        do {
                            ...
                            print("[ERROR] Failed to open with application: \(error.localizedDescription)")
                        }
                }
                return
            }
            ...
            case "Compress":
                ...
                Task {
                    do {
                        ...
                        print("[ERROR] Compress failed: \(error)")
                    }
                }
        ...
        private func openWithPanel() {
            ...
            let enableLabel = NSTextField(labelWithString: AppLocalizer.localized("Enable:"))
            ...
            popup.addItems(withTitles: [
                AppLocalizer.localized("Recommended Applications"),
                AppLocalizer.localized("All Applications")
            ])
            ...
            let alwaysCheckbox = NSButton(
                checkboxWithTitle: AppLocalizer.localized("Always Open With"),
                target: nil,
                action: nil
            )
                        ...
                        do {
                            ...
                            if alwaysCheckbox.state == .on, let bundleID = Bundle(url: appURL)?.bundleIdentifier {
                                if let contentType = (try? fileURL.resourceValues(forKeys: [.contentTypeKey]))?.contentType {
                                    ...
                                    if status != noErr { print("[WARN] Failed to set default handler for \(contentType.identifier): \(status)") }
                                } else if let scheme = fileURL.scheme {
                                    let status = LSSetDefaultHandlerForURLScheme(scheme as CFString, bundleID as CFString)
                                    if status != noErr { print("[WARN] Failed to set default handler for scheme \(scheme): \(status)") }
                                }
                            }
        ...
        private func handleRemoveBackground() {
            ...
            Task {
                do {
                    ...
                    print("[ERROR] Failed to remove background: \(error.localizedDescription)")
                    await showErrorAlert(title: AppLocalizer.localized("Background Removal Failed"), message: error.localizedDescription)
                }
            }
        }
        ...
        private func handleCreatePDF() {
            ...
            Task {
                do {
                    ...
                    print("[ERROR] Failed to create PDF: \(error.localizedDescription)")
                    await showErrorAlert(title: AppLocalizer.localized("PDF Creation Failed"), message: error.localizedDescription)
                }
            }
        }
        ...
        private func showConvertImageDialog() {
            ...
            let alert = NSAlert()
            alert.messageText = AppLocalizer.localized("Convert Image")
            alert.alertStyle = .informational
            alert.addButton(withTitle: AppLocalizer.localized("Convert"))
      ...
      // MARK: Format Row
            let formatLabel = NSTextField(labelWithString: AppLocalizer.localized("Format:"))
      ...
      // MARK: Image Size Row
            let imageSizeLabel = NSTextField(labelWithString: AppLocalizer.localized("Image Size:"))
            ...
            imageSizePopup.addItems(withTitles: [
                AppLocalizer.localized("Actual Size"),
                AppLocalizer.localized("Large"),
                AppLocalizer.localized("Medium"),
                AppLocalizer.localized("Small"),
                AppLocalizer.localized("Custom...")
            ])
            ...
            customSizeField.placeholderString = AppLocalizer.localized("e.g., 1920")
      ...
      // MARK: Preserve Metadata Checkbox
            let metadataCheckbox = NSButton(
                checkboxWithTitle: AppLocalizer.localized("Preserve Metadata"),
                target: nil,
                action: nil
            )
      ...
      // MARK: Format-specific options (shown/hidden based on format selection)
            ...
            let qualityLabel = NSTextField(labelWithString: AppLocalizer.localized("Compression:"))
            ...
            if response == .alertFirstButtonReturn {
                ...
                Task {
                    do {
                        ...
                        print("[ERROR] Failed to convert image: \(error.localizedDescription)")
                        showErrorAlert(title: AppLocalizer.localized("Image Conversion Failed"), message: error.localizedDescription)
                    }
                }
            }
        }
        ...
        private func showErrorAlert(title: String, message: String) {
            ...
            alert.addButton(withTitle: AppLocalizer.localized("OK"))
            alert.runModal()
        }
    }
