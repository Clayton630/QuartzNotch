...
private final class FullDiskAccessPermissionStore: ObservableObject {
    ...
    func refreshStatus() {
        ...
        let status = FullDiskAccessAuthorization.refreshPermissionStatus(markAsProbed: true)
    ...
    func requestAccessPrompt() {
        ...
        alert.icon = appIcon
        alert.messageText = AppLocalizer.localized("Full Disk Access Required")
        alert.informativeText = AppLocalizer.localized("QuartzNotch needs Full Disk Access to read Focus metadata (custom icon and color). Click Continue, then add QuartzNotch in Privacy & Security > Full Disk Access.")
        alert.alertStyle = .informational
        alert.addButton(withTitle: AppLocalizer.localized("Continue"))
